...
using PricingIndexViewModel = front_to_back.ViewModels.Pricing.PricingIndexViewModel;
...
namespace front_to_back.Areas.Admin.Controllers
{
    ...
    public class PricingController : Controller
    {
        ...
        public async Task<IActionResult> Create(PricingComponent pricing)
        {
            ...
            bool isExist = await _appDbContext.PricingComponents.AnyAsync(c => c.Title.ToLower().Trim() == pricing.Title.ToLower().Trim());

            if (isExist)
            {
                ModelState.AddModelError("Title", "This category already declared");
        ...
        public async Task<IActionResult> Update(int id, PricingComponent pricingComponent)
        {
            ...
            if (isExist)
            {
                ModelState.AddModelError("Title", "This category was declared");
                return View(pricingComponent);
            }

            pricing.Title = pricingComponent.Title;
            pricing.Description = pricingComponent.Description;
            pricing.Price = pricingComponent.Price;
